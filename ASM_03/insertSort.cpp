...
//
// Threads programming example for Linux (03/2021)
...
public:
    ...
    for (int i = (m_from + 1); i <= m_length; i++)
    {
        
        item = m_data[i];
        j = i-1;
        while ((j >= m_from) && (m_data[j] > item))
        {
            m_data[j+1] = m_data[j];
            j--; 
        }
    ...
    printf("Zoradené pole od %d s dlzkou %d:\n", m_from, m_length);
    for (int i = m_from; i < m_length; i++)
    {
        printf("%d,", m_data[i]);
    }
        printf("\n");
